...
class Diameter:
    ...
    def decodeAvpPacket(self, data):
        ...
        while len(data) > 16:
            try:
                ...
                print(e)
                self.logTool.log(service='HSS', level='error', message=f"An error occurred: {e}", redisClient=self.redisMessaging)
    ...
    def getDiameterMessageType(self, binaryData: str) -> dict:
        ...
        for diameterApplication in self.diameterResponseList:
            try:
                if packet_vars["command_code"] != diameterApplication["commandCode"]:
    ...
    def sendDiameterRequest(self, requestType: str, hostname: str, **kwargs) -> str:
        ...
        try:
            ...
            for diameterApplication in self.diameterRequestList:
                try:
                    if requestType != diameterApplication["requestAcronym"]:
            ...
            try:
                ...
                for diameterApplication in self.diameterResponseList:
                    try:
                        ...
                        if packet_vars["ApplicationId"] != diameterApplication["applicationId"]:
                            raise ValueError("Invalid Application ID")
                        if 'flags' in diameterApplication and str(packet_vars["flags"]) != str(diameterApplication["flags"]):
